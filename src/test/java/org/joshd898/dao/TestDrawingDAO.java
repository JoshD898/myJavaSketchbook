...
public class TestDrawingDAO extends AbstractDatabaseTest {
    ...
    void testUpdateDrawing() {
        try {
            ...
            Drawing drawingFromDB = drawingDAO.loadDrawing(d1.getDrawingID());

            assertEquals(d1.getTitle(), drawingFromDB.getTitle());
            assertEquals(d1.getWidth(), drawingFromDB.getWidth());
            assertEquals(d1.getHeight(), drawingFromDB.getHeight());
            
            for (int x = 0; x < d1.getWidth(); x++) {
                for (int y = 0; y < d1.getHeight(); y++) {
                    assertEquals(d1.getRGB(x, y), drawingFromDB.getRGB(x, y));
                }
            }
        } catch (SQLException | IOException e) {
            fail(e.getMessage());
